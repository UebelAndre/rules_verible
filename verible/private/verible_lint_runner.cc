...
///     (`--verible-lint=`, `--src=`, `--config=`) the same way. This path
...
///   | `--config=PATH`         | (optional) Passed through as `--rules_config=PATH`.  |
...
int main(int argc, char** argv) {
  ...
  if (!config.empty()) cmd.push_back("--rules_config=" + config);
