/// @file
/// @brief Process wrapper invoked by the `verible_format` aspect and test rule.
///
/// @details
///   Runs `verible-verilog-format --verify` against a list of source files and,
///   on success, touches a marker file so Bazel records the action as cached.
///   When a non-zero exit code is observed the captured stdout/stderr from the
///   child is forwarded to stderr.
///
...
[[noreturn]] void die(const std::string& msg) {
  std::fprintf(stderr, "verible_format_runner: %s\n", msg.c_str());
  std::exit(2);
}

/// @brief Load one argument per line from a Bazel parameter file.
/// @param path Filesystem path to the param file (without the leading `@`).
...
/// @details Translates args-file values (rlocation keys) into absolute paths
///          under the test's runfiles tree. In aspect/CLI mode (no runfiles
///          object), the key is already a usable sandbox path so it is returned
///          verbatim.
...
int main(int argc, char** argv) {
  ...
  std::string test_args_key = sp::get_env("VERIBLE_FORMAT_TEST_ARGS_FILE");
  if (!test_args_key.empty()) {
    ...
    if (args_file_path.empty()) {
      die("could not resolve VERIBLE_FORMAT_TEST_ARGS_FILE='" + test_args_key + "'");
    }
  ...
  for (const auto& a : args) {
    ...
    if (a == "--") {
      after_dashdash = true;
    } else if (starts_with(a, "--verible-format=")) {
      verible_bin = maybe_resolve(runfiles.get(), flag_value(a, "--verible-format"));
  ...
  if (verible_bin.empty()) die("--verible-format=PATH is required");
